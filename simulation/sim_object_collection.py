...
from .interface import ISimObjectCollection, ISimObject
...
class QuadTree(object):
    ...
    .rect (pygame.Rect) attribute that is a pygame.Rect ... and they must be hashable.
...
class SimulationObjectCollection(ISimObjectCollection):

    _qt: QuadTree

    def compile(self) -> None:
        self._qt = QuadTree(self._members, bounding_rect=self._boundary)
    ...
    def collides_with(self, rect: pygame.Rect) -> List[ISimObject]:
        result: List[ISimObject] = list(self._qt.hit(rect))
        return result
    ...
    def __init__(self, boundary: Rect):
        self._boundary: Rect = boundary
        self._members: List[ISimObject] = list()
